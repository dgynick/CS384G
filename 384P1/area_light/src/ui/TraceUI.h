...
public:
	...
	bool	smShadSw() const { return m_smoothshade; }
        bool    getUsingCubeMap() const { return m_usingCubeMap; }
	int     getKdMaxLeafSize() const { return kdMaxLeafSize; }
        int     getKdMaxDepth() const{ return kdMaxDepth;}
        int     getNumOfRays() const{ return numOfRays;}
	static bool m_debug;
...
protected:
	...
	int	m_nDepth;	// Max depth of recursion
        int     m_nThreads = 1;     // Number of threads being used
        int     m_nSupersamplingPixels = 1; //number of pixels used in supersampling
