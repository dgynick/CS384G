      ...
      if(depth > 0){
	    ...
	    colorC += refl;*/
            ...
            for(int k =0; k<traceUI->getNumOfRays(); k++){
                 ...
                 //cout<<"sampling"<<u1<<u2<<endl;
                 
                 double phi = atan(ALPHA_Y/ALPHA_X * tan(2 * PI * u2));
                 double delta = sqrt(-log(u1)/(cos(phi)*cos(phi)/ALPHA_X/ALPHA_X + sin(phi) * sin(phi)/ALPHA_Y/ALPHA_Y));
                
                 Vec3d h = cos(phi) * i.tangent + sin(phi) * binorm + i.N/tan(delta);
                 h.normalize();
                 Vec3d r = d + 2 * (-d * h) * h;
