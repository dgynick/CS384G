...
Vec3d Material::shade(Scene *scene, const ray& r, const isect& i) const
{
  ...
  Vec3d Q = r.at(i.t);
  Vec3d color = ke(i) + ka(i) * scene -> ambient();
  Vec3d N = i.N;
  ...
  Vec3d tempc;
  for ( std::vector<Light*>::const_iterator itr = scene -> beginLights(); itr != scene -> endLights(); itr++){
    Light* l = *itr;
    if((l -> getDirection(Q)) * N < 0){
      continue;
    }
    Vec3d shadow = l -> shadowAttenuation(r, Q);
    Vec3d atten = shadow * l -> distanceAttenuation(Q);
    //cout<< "shadow attenuation: "<< shadow << endl;
    tempc = l->getColor();
    tempc *= atten;
...
Vec3d TextureMap::getMappedValue( const Vec2d& coord ) const
{
  ...
  double deltax = u - floor(u);
  double deltay = v - floor(v);

  Vec3d color;
  color += deltax * deltay * getPixelAt(ceil(u), ceil(v));
  color += deltax * (1 - deltay) * getPixelAt(ceil(u), floor(v));
  color += (1 - deltax) * deltay * getPixelAt(floor(u), ceil(v));
  color += (1 - deltax) * (1 - deltay) * getPixelAt(floor(u), floor(v));
